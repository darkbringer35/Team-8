...
public class ChickenDialog extends JDialog implements ActionListener{
	...
	//재고관리 변수선언
	JTextField menuTxt ; //메뉴명 텍스트필드
	JTextField stockTxt ; //재고 텍스트필드
	JTextField priceTxt ; //가격 텍스트필드
	...
	public void ItemManagerUI(){
		...
		JComboBox cb = new JComboBox(comboIndex); //메뉴번호 콤보박스
		menuTxt = new JTextField(); //메뉴명 텍스트필드
		stockTxt = new JTextField(); //재고 텍스트필드
		priceTxt = new JTextField(); //가격 텍스트필드
		//dao.getComboIndex();
		...
		//재고 목록 TextField 구성
		ta = new JTextArea(10,40); //ta영역 생성
